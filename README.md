...
DCreel 是一个面向 Windows 的本地桌面整理工具，使用 Rust、Tauri 2、React 和 TypeScript 构建。每个桌面盒子都映射到用户选择的真实文件夹；新建收纳盒时会在自选位置创建文件夹，新建映射盒子则直接使用已有目录。
...
5. `UserAction`：原生盒子菜单上报新建、重命名、收起、锁定、换色、移除和彻底退出操作，主程序复用现有命令和持久化路径执行。
...
1. 枚举 `Progman` 和全部 `WorkerW`，逐个尝试当前可访问的桌面宿主层级。
2. 用 `GW_HWNDPREV` 找到宿主正上方的 Z 序位置，再依次插入所有 DCreel 盒子；Explorer 尚未稳定时保留 Host 并快速重试，不再反复重启进程。
...
- 导入遇到同名文件时使用 `name (2).ext`、`name (3).ext` 等新名称，不覆盖现有内容。
- 批量导入会先完成全部路径和目标名称检查；中途失败时自动把本批已移动项目移回原处，并记录无法回滚的异常。
- 文件跨磁盘卷移动时会使用独占目标复制，完整写入后才删除源文件。
- 桌面 DCreel 右键子菜单和盒子空白区域右键菜单均提供“退出 DCreel”，隐藏托盘后无需通过任务管理器结束程序。

## 开发环境

建议在 Windows 10/11 上准备：

- Node.js 22.12 或更高版本（当前工程已在 Node.js 24 验证）。
...
src-tauri/
  ...
  src/commands.rs      文件导入/打开/定位、快速新建与配置命令
